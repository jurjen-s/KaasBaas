...
-- Fri 27 Jan 2017 04:30:47 PM CET
...
CREATE TABLE IF NOT EXISTS `mydb`.`klanten` (
  ...
  INDEX `FK_adressen_klant_idx` (`FK_adressen_klant` ASC),
  CONSTRAINT `FK_accounts_id`
    FOREIGN KEY (`FK_accounts_id`)
    REFERENCES `mydb`.`accounts` (`accounts_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_adressen_id`
    FOREIGN KEY (`FK_adressen_klant`)
...
CREATE TABLE IF NOT EXISTS `mydb`.`bestellingen` (
  ...
  INDEX `FK_adressen_bestelling_idx` (`FK_adressen_id` ASC),
  CONSTRAINT `FK_klanten_id`
    FOREIGN KEY (`FK_klanten_id`)
    REFERENCES `mydb`.`klanten` (`klanten_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_adressen_id`
    FOREIGN KEY (`FK_adressen_id`)
    REFERENCES `mydb`.`adressen` (`adressen_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`facturen` (
  ...
  INDEX `FK_adressen_factuur_idx` (`FK_adressen_factuur` ASC),
  INDEX `FK_klanten_id_idx` (`FK_klanten_id` ASC),
  CONSTRAINT `FK_bestellingen_id`
    ...
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_adressen_id`
    FOREIGN KEY (`FK_adressen_factuur`)
    REFERENCES `mydb`.`adressen` (`adressen_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_klanten_id`
